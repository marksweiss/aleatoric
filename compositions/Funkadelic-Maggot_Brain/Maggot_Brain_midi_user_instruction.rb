...
PLAYER_SETTINGS = {
  ...
  "transpose_shift_down_factor" => -1,
  # Factor for shift up, likewise (1.0 in CSound, 1 in MIDI)
  "transpose_shift_up_factor" => 1,  
...
class In_C_Player
  ...
  def swing_adj
    if swing?(@swing_base_val, @swing_num_steps) then
      ...
      start_swing = swing(@swing_base_val, @swing_num_steps, @swing_step_size)
      dur_swing = swing(@swing_base_val, @swing_num_steps, @swing_step_size)
...
instruction_8 = lambda do |container, score|
  ...
  score.notes.each do |note| 
    note.start(note.start * start_swing)
    # note.start(0.0) if note.start < 0.0
    note.duration(note.duration * dur_swing)
