...
# Returns all script lines preprocesed and joined into one string
def optional_preprocess_script(script_lines, src_file_name)
  ComposerAST.new.optional_preprocess_script(script_lines, src_file_name)
end
...
def write_test_script(script, lite_syntax=false)
  ...
  preproc_script = "test_preprocess.altc" 
  File.open(preproc_script, "w") {|f| script.each {|line| f << line}}
  script_lines = File.readlines preproc_script
  script_lines = ComposerAST.new.mandatory_preprocess_script(script_lines)
  if lite_syntax
    # Returns all script lines preprocesed and joined into one string
    script = optional_preprocess_script(script_lines, preproc_script)
  else
...
write "composer_test_results.txt"
  ...
  tester, results = test_runner(test_name, throw_on_failure, script, lite_syntax)
  actual = results
  # Loop exit is postplay so the loop above produces one note and then exits the loop
  expected0 = 'i 1 1.00000 0.20000 1100 7.02000 1 ;'
  tester.assert(expected0 == actual[2])
  puts tester.to_s  
end
