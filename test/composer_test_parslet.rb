...
def parslet_xform_composer_to_ruby(script, lite_syntax=false)
  return script if lite_syntax
  
  ret = ''
  begin
    parser = ComposerParser.new
    xformer = ComposerTransformer.new
    parse_tree = parser.parse(script)
    ret = xformer.apply(parse_tree)    
...
note "note 1"
  ...
  tester, actual = parslet_test_runner(test_name, throw_on_failure, script, lite_syntax)
  actual = parslet_xform_composer_to_ruby(script, lite_syntax)
  
  # TEMP DEBUG
  p actual
  
  expected = 'note "note 1" do\r\n'  
  tester.assert(expected == actual)
  puts tester.to_s  
end

def parslet_run_tests(flags='run_all')    
  ...
  if not run_only
    ...
    begin
      #
      parslet__test__comment ; num_tests += 1       
...
def write_test_script(script, lite_syntax=false)
  ...
  # TODO THIS IS THE NEW FUNCTION TO GET WORKING
  script = xform_composer_to_ruby(script, lite_syntax)
  
  # TEMP DEBUG
  p script
  
  # TODO THIS IS THE OLD CODE TO BE REPLACED
